...
docker run -d --name nginx1 -p 81:80 --mount type=bind,source=$(pwd)/shared1,target=/usr/share/nginx/html nginx:latest
docker run -d --name nginx2 -p 82:80 --mount type=bind,source=$(pwd)/shared2,target=/usr/share/nginx/html nginx:latest
